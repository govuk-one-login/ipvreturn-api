...
jest.mock("../../utils/Config", () => ({
	getParameter: jest.fn(() => "client-id"),
}));
...
describe("SessionHandler", () => {
	...
	it("returns a not found response for an invalid resource request", async () => {
	...
	it("returns success response for a valid request payload", async () => {
		...
		mockedSessionProcessor.processRequest.mockResolvedValue({
			statusCode: 200,
