...
describe("post event processor", () => {
	...
	it("when all 3 events are sent, a Dynamo record with the details of all three events populated", async () => {
		await postMockEvent(VALID_F2F_YOTI_START_TXMA_EVENT, userId);
		await postMockEvent(VALID_AUTH_IPV_AUTHORISATION_REQUESTED_TXMA_EVENT, userId);
		await postMockEvent(VALID_IPV_F2F_CRI_VC_CONSUMED_TXMA_EVENT, userId);
