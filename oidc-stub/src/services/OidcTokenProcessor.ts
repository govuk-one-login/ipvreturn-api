...
export class OidcTokenProcessor {
    ...
    async generateToken(event: any): Promise<any> {    	
		...
		if (config.signingKey) {
			...
			const signedJwt = await this.sign(payload, config.signingKey);
	...
	async sign(jwtPayload: JarPayload, keyId: string): Promise<string> {
		...
		const params = {
			...
			KeyId: kid,
			SigningAlgorithm: AWS.SigningAlgorithmSpec.RSASSA_PKCS1_V1_5_SHA_256,
