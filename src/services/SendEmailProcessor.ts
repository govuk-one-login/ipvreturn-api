...
export class SendEmailProcessor {
  ...
  constructor(
  	...
  	this.govNotifyService = SendEmailService.getInstance(
  		this.logger,
  		GOVUKNOTIFY_API_KEY,
  		govnotifyServiceId,
  	);
  	this.sessionEventsTable = sessionEventsTable;
  	this.iprService = IPRServiceSession.getInstance(
  ...
  static getInstance(
  	...
  	if (!SendEmailProcessor.instance) {
  		SendEmailProcessor.instance = new SendEmailProcessor(
  			logger,
  			metrics,
  			GOVUKNOTIFY_API_KEY,
  			govnotifyServiceId,
  ...
  async processRequest(
  	...
  	try {
  		...
  		this.logger.error(
  			{ message: "getSessionByUserId - failed executing get from dynamodb:" },
  	...
  	if (!session.notified) {
  		this.logger.error(
  			...
  			{ messageCode: MessageCodes.NOTIFIED_FLAG_NOT_SET_TO_TRUE },
  		);
  		throw new AppError(
  			HttpCodesEnum.SERVER_ERROR,
  			"Notified flag is not set to true for this user session event",
      ...
      message instanceof DynamicEmail
  	...
  	if (message.messageType !== Constants.VISIT_PO_EMAIL_FALLBACK) {
  		...
  		data = await this.validationHelper.validateSessionEvent(
  			sessionEventData,
  			message.messageType,
  			this.logger,
  		);
  	...
  	const emailResponse: EmailResponse = await this.govNotifyService.sendEmail(
  		message,
  		data.emailType,
  	);
  	await this.iprService.sendToTXMA({
  		...
  		...buildCoreEventFields(
  			{ email: message.emailAddress, user_id: message.userId },
