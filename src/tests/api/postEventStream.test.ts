import {
	...
	VALID_IPV_F2F_DOCUMENT_UPLOADED_TXMA_EVENT,
...
describe("post event processor", () => {
	...
	it("when all 3 events are sent, a Dynamo record with the details of all three events populated", async () => {
		...
		await postMockEvent(VALID_IPV_F2F_DOCUMENT_UPLOADED_TXMA_EVENT, userId, false);
		...
		expect(response?.documentExpiryDate).toBe("2030-11-21");
