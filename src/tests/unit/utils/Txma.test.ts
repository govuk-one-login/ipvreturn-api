...
const mockEnvironmentVariables = { componentId: () => mockComponentId } as Pick<
...
describe("TxmaEvents", () => {
  ...
  describe("buildCoreEventFields", () => {
    it("Returns object with default values user, timestamp, and component_id", () => {
      expect(
        buildCoreEventFields({ user_id }, mockEnvironmentVariables),
      ).toEqual({
        user: { user_id },
    ...
    it("Returns object with user with email if provided", () => {
      expect(
        buildCoreEventFields({ user_id, email }, mockEnvironmentVariables),
      ).toEqual({
        user: { user_id, email },
        timestamp: 1585695600,
        event_timestamp_ms: 1585695600000,
