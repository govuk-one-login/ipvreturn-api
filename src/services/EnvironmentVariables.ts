...
export class EnvironmentVariables {
  ...
  private readonly GOVUKNOTIFY_TEMPLATE_ID =
    process.env.GOVUKNOTIFY_TEMPLATE_ID;

  private readonly GOVUKNOTIFY_DYNAMIC_EMAIL_TEMPLATE_ID =
    process.env.GOVUKNOTIFY_DYNAMIC_EMAIL_TEMPLATE_ID;

  private GOVUKNOTIFY_MAX_RETRIES = process.env.GOVUKNOTIFY_MAX_RETRIES;

  private GOVUKNOTIFY_BACKOFF_PERIOD_MS =
    process.env.GOVUKNOTIFY_BACKOFF_PERIOD_MS;

  private readonly GOVUKNOTIFY_API_KEY_SSM_PATH =
  ...
  private readonly SESSION_RETURN_RECORD_TTL_SECS =
    process.env.SESSION_RETURN_RECORD_TTL_SECS;
  ...
  private readonly ASSUMEROLE_WITH_WEB_IDENTITY_ARN =
    process.env.ASSUMEROLE_WITH_WEB_IDENTITY_ARN;

  private OIDC_JWT_ASSERTION_TOKEN_EXP =
    process.env.OIDC_JWT_ASSERTION_TOKEN_EXP;
  ...
  private readonly GOVUKNOTIFY_FALLBACK_EMAIL_TEMPLATE_ID =
    process.env.GOVUKNOTIFY_FALLBACK_EMAIL_TEMPLATE_ID;
    ...
    switch (serviceType) {
      case ServicesEnum.GOV_NOTIFY_SERVICE: {
        if (
          ...
          !this.GOVUKNOTIFY_API ||
          this.GOVUKNOTIFY_API.trim().length === 0 ||
          !this.GOVUKNOTIFY_DYNAMIC_EMAIL_TEMPLATE_ID ||
          this.GOVUKNOTIFY_DYNAMIC_EMAIL_TEMPLATE_ID.trim().length === 0 ||
          !this.GOVUKNOTIFY_TEMPLATE_ID ||
      ...
      case ServicesEnum.GET_SESSION_EVENT_DATA_SERVICE: {
        if (
          ...
          this.SESSION_EVENTS_TABLE.trim().length === 0 ||
          !this.OIDC_URL ||
          this.OIDC_URL.trim().length === 0 ||
          !this.RETURN_REDIRECT_URL ||
          this.RETURN_REDIRECT_URL.trim().length === 0 ||
          !this.ASSUMEROLE_WITH_WEB_IDENTITY_ARN ||
