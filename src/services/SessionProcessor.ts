...
export class SessionProcessor {
	...
	async processRequest(event: APIGatewayProxyEvent): Promise<Response> {
		...
		try {
			...
			// from the ID token
			const iprService = IPRServiceSession.getInstance(this.environmentVariables.sessionEventsTable(), this.logger, createDynamoDbClientWithCreds(assumedRole.Credentials));
