...
import {
  VALID_GOV_NOTIFY_HANDLER_SQS_EVENT,
  VALID_GOV_NOTIFY_HANDLER_SQS_EVENT_DYNAMIC_EMAIL,
...
describe("SendEmailProcessor", () => {
  beforeAll(() => {
    ...
    // @ts-ignore
    sendEmailProcessorTest.environmentVariables = {
  ...
  it.each(["userId", "firstName", "lastName", "emailAddress"])(
    "Throws error when event body message is missing required attributes to send static template Email",
    async (attribute) => {
      const eventBody = JSON.parse(sqsEvent.Records[0].body);
  ...
  it.each(["userId", "firstName", "lastName", "emailAddress"])(
    "Throws error when the Email modal validation fails",
    async (attribute) => {
      const eventBody = JSON.parse(sqsEvent.Records[0].body);
      const eventBodyMessage = eventBody.Message;
  ...
  it.each(["journeyWentAsyncOn", "ipvStartedOn", "readyToResumeOn"])(
    "Throws error when session event record is missing necessary Event timestamps fields",
    async (attribute) => {
      const eventBody = JSON.parse(sqsEvent.Records[0].body);
      const eventBodyMessage = eventBody.Message;
      eventBody.Message = eventBodyMessage;
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        `${attribute} is not yet populated, unable to process the DB record.`,
        { messageCode: "MISSING_MANDATORY_FIELDS_IN_SESSION_EVENT" },
      );
    },
  );

  it.each(["userEmail", "nameParts", "clientName", "redirectUri"])(
    "Throws error when session event record is missing necessary field %s required to send static template Email",
    async (attribute) => {
      const eventBody = JSON.parse(sqsEvent.Records[0].body);
      const eventBodyMessage = eventBody.Message;
      eventBody.Message = eventBodyMessage;
      ...
      expect(logger.error).toHaveBeenCalledWith(
        "The mandatory fields required for static template email are missing in session record, trying to send the fallback template email.",
        { messageCode: "MISSING_MANDATORY_FIELDS_IN_SESSION_EVENT" },
      );
    },
  );
  ...
  it("Throws error when notified flag is not set to true for the user session event record", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      "Notified flag is not set to true for this user session event",
      { messageCode: "NOTIFIED_FLAG_NOT_SET_TO_TRUE" },
    );
  });
  ...
  it("Throws error when the session event record was not found for the userId", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      "No session event found for this userId",
  ...
  it("Returns success response when all required Email attributes exists to send newEmail messageType", async () => {
    ...
    const newEmailmessage = DynamicEmail.parseRequest(
      JSON.stringify(eventBody.Message),
    );
    const emailResponse =
      await sendEmailProcessorTest.processRequest(newEmailmessage);

    expect(mockGovNotifyService.sendEmail).toHaveBeenCalledWith(
      newEmailmessage,
      Constants.VIST_PO_EMAIL_DYNAMIC,
    );
  ...
  it.each([
    ...
    "when session event record is missing necessary field %s required to send newEmail, then it falls back to sending static template Email",
    async (attribute) => {
      const expectedDateTime = new Date().toISOString();
      const mockEmailResponse = new EmailResponse(expectedDateTime, "", 201);
      mockGovNotifyService.sendEmail.mockResolvedValue(mockEmailResponse);
      ...
      expect(logger.info).toHaveBeenNthCalledWith(
        2,
        "Unable to process the DB record as the necessary fields to send the dynamic template email are not populated, trying to send the static template email.",
        { messageCode: "MISSING_NEW_PO_FIELDS_IN_SESSION_EVENT" },
      );
      expect(mockGovNotifyService.sendEmail).toHaveBeenCalledWith(
        newEmailmessage,
        Constants.VIST_PO_EMAIL_STATIC,
      );
  ...
  it("Includes component_id in the TxMA event when sending an email", async () => {
    const expectedDateTime = new Date().toISOString();
    const mockEmailResponse = new EmailResponse(expectedDateTime, "", 201);
    mockGovNotifyService.sendEmail.mockResolvedValue(mockEmailResponse);
    const eventBody = JSON.parse(sqsEvent.Records[0].body);
    ...
    await sendEmailProcessorTest.processRequest(message);
